...
const loadInitialData = (currentUser) => {
  const { owner, transactions } = currentUser; // Destructure the owner from currentUser
  displayTransactions(currentUser, false);
  displaySummary(transactions);
  displayBalance(transactions);
  currentUserLabel.textContent = owner; // Set currentUserLabel to the full name of the owner
